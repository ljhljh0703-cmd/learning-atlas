...
내가 매일 쓰던(2026-06 당시, 이후 Gemini 폐기) Claude Code + Gemini + ChatGPT 세션이 **불투명한 JSONL로 흩어져 있는 문제**를 정확히 정조준한다. Sub brain의 `wiki/` ↔ `raw/` 2계층 구조를 그대로 차용한 동족 프로젝트.
...
→ 내가 쓰던(당시) **Claude+Gemini 분업** (professional에 명시) 양쪽 로그를 같은 인덱스에 합칠 수 있음.
...
- professional — 분업(당시 Claude+Gemini) 양쪽 로그를 합칠 수 있는 도구
