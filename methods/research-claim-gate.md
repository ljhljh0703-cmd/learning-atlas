...
- [Pattern: Structured Output Gate (구조화 출력 게이트)](prompt-pattern-structured-output-gate.md) · [Forge Spec-Gate (why-was-fable-banned) — 차용 해체](forge-spec-gate.md) · [Playwright E2E 에이전트 하네스 — 테스트=실행가능 명세, trace=user-facing 증빙 매체 (Naver 발표)](playwright-e2e-agent-harness.md) — 게이트 기반 검증 방법론 자매(본 페이지=증거 등급·주장 게이트 / 구조 강제 / spec=test / E2E 관측)
