...
## 적용처 (Sub brain 운영)
- **모든 AI 운영**: 위 패턴은 Codex/Gemini/Claude 디스패치 프롬프트에 직접 적용 가능(특히 패턴 #6 경계명시(assessment vs fix)·#5 진행근거·#4·#10 체크포인트는 외부 에이전트 RETURN 품질↑).
- **하네스 진화**: v1 보강 입력(공식 대조).
- 본 vault CLAUDE.md/GEMINI.md 운영 규칙과 상당 부분 이미 정합(메모리·정직보고·과계획억제) — 공식 확증.
