...
## §dedup2-2차 — 2차 검토 2026-09-02 (규율 **유지**)

발동 = `dedup2` **25건 누적**(CLAUDE.md §4 review_trigger 「25건 누적 시 또는 hit 3연속 0」).

### 실측

| | |
|---|---:|
| 헌법 명령 `grep -c "· dedup2:"` 결과 | **25** |
| 실제 occurrence | 26 |
| 그중 **원장 자기 설명문**(헤더 §형식 설명) | **2** |
| **실제 계측 기록** | **24** |
| 판정 가능(hit/miss) | 17 |
| `n/a`(신규 개념 노트 0 = 대상 아님) | 6 |
| `생략` | 1 |

**hit 12 / miss 5 = 71%.** hit 3연속 0 조건은 근처에도 안 갔다(최장 연속 miss = 1).

### 판정 = 유지

폐기 근거가 없다. hit 중 **2건은 결정을 통째로 뒤집었다** — ① 08-25: 1패스가 불교 키워드 4종에 전건 0을 내 「vault 에 불교 지식 없음」으로 끝낼 뻔했고, 2패스 확장이 `atlas/east` 불교 인물 5건을 잡아 **배치 결정(신설이냐 병합이냐)을 바꿨다.** ② 08-25(cosmos): 확장 grep 이 `cosmos-e1-cards` 를 물어 **「cosmos 흡수 0」이라는 보고 자체가 뒤집혔다.** 한 번이라도 이 급을 막으면 2패스 비용(grep 2~3회)은 회수된다.

### miss 5건의 성격 — 실패가 아니라 3종이다

| 유형 | 건 | 뜻 |
|---|---:|---|
| **1패스로 이미 충분** | 1 (08-25 HCL) | 확장이 헛돌았으나 1패스가 잡았다. 비용만 발생 |
| **확장이 오탐만** | 1 (08-17 스크롤) | 확장어가 `package` 오탐으로 수렴 |
| **grep 이 구조적으로 못 잡는 중복** | 1 (09-02 WikiSkill) | ↓ |
| 사유 미기재 | 1 (07-30) | 라벨만 있고 근거 없음 = 계측 위생 결함 1건 |
| (나머지 1건 = 08-13 ReasoningBank) | 1 | 확장이 기존 memory 노드 6종을 보였으나 신설 판정 유지 |

**세 번째 유형이 이번 검토의 유일한 새 관측이다.** 09-02 WikiSkill 흡수에서 1·2패스 모두 0이었는데, 실제 중복 위험은 **반영처 노트 136행에 이미 있던 인접 확장 후보**(`aces-skill-evaluation` 의 `H-iso/H-group`)였다. **grep 은 「어느 파일인가」를 답하지 「그 파일 어디를 봐야 하는가」를 답하지 않는다.** 판정을 바꾼 건 반영처 노트를 끝까지 읽은 것이었다.

> ⚠️ **그렇다고 3패스를 신설하지 않는다.** n=1 이고, 「반영처 노트를 읽는다」는 규율로 못박을 것이 아니라 당연한 절차다. 규율 비대화(Karpathy #2)가 이 관측 1건의 값보다 비싸다. **다음 miss 에서 같은 유형이 또 나오면 그때 재판단한다.**

### 발견된 결함 1건 — 집계 명령이 자기 설명문을 센다

헌법 §4 가 지정한 집계 명령 `grep -c "· dedup2:" wiki/use-ledger.md` 는 **원장 헤더의 §형식 설명 2줄을 계측으로 센다**(토큰을 설명하려면 토큰을 적어야 하므로 구조적이다). 그리고 `grep -c` 는 *줄* 을 세므로 occurrence 26 ⟂ 줄 25 도 어긋난다.
→ **트리거가 실제 24건에서 발화했다.** 방향이 안전한 쪽(조기 발화)이라 이번 판정을 바꾸지 않지만, 임계가 값을 뜻하려면 분모가 정확해야 한다. 정정안 = `스테이징 영역`(스테이징 — 임계 발화 시점을 바꾸므로 직접 수정 안 함).

### ✅ 작가 확정 2026-09-02 — 「중복검사 유지」

작가 명시 컨펌(결정 본문 인용 = 「유지」). `review_trigger` 는 아래로 교체했다(헌법 §4 반영):
**누적 50건** 또는 **hit 3연속 0** 또는 **「grep 이 못 잡는 중복」 유형 2건째 발생 시**.
※ 임계 25→50 상향은 *분모 정확도 패치*(`dedup2-count-command-2026-09-02`)와 독립이다 — 그 패치가 머지되면 분모가 2 줄어들 뿐 임계는 그대로다.
...
> ⚠ 정정 2026-09-05(자문 F07): 아래 역사 원문 속 개념 조회 명령(`grep -c "…"`)은 실행 오류가 나는 옛 형태다. **현행 = `grep -Fc -- '<개념>' …`**, exit 1 = 무일치 정상 · 2+ = 오류. 정본 = CLAUDE.md §4 역인출. 원문은 역사 보존.
