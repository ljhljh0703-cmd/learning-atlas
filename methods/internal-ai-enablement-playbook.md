...
### 0.1 ⭐ AX Coupled-Loop Principle (NDC26 대담 델타, 2026-08-04 ③Gate)

> **바텀업과 탑다운은 서로 다른 운영체제가 아니라 서로 다른 시작점이다. 성공한 AX는 결국 `현장 탐색 루프`와 `중앙 거버넌스 루프`를 결합한다.**

넥슨(현업 실험·리터러시·챔피언 전파) ⟂ 크래프톤(경영진 선언·공통 인프라·비용 대시보드)은 겉으로 정반대지만 **같은 구조로 수렴한다** — 중앙 조직이 인프라·현업 브리지 / 작은 업무에서 먼저 효용 / 현업이 직접 만들거나 고름 / 비용·보안·피로를 운영 문제로 계측 / **사용자가 늘지 않는 도구는 성공으로 보지 않음**.

- **Loop A(현장 탐색)** 없으면 중앙 조직은 *사용되지 않는 도구 공장*이 된다.
- **Loop B(중앙 거버넌스)** 없으면 현장 실험은 *섀도우 AI·도구 동물원·비용 파편화·보안 부채*로 번진다.
- **Coupler = 중앙 조직의 제품은 단일 챗봇이 아니라 `실험→증거→확산→표준화/폐기` 경로다.**

**승격/폐기 퍼널** (application delta — 이 노트에 가장 약했던 슬롯):

```text
Pain observed → local prototype → user repeats it → measurable gain
→ cost/risk check → cross-context reuse → Promote / Keep local / Park / Retire
```

핵심 질문은 `만들었는가`가 아니라 ① **누가 다음 주에도 다시 여는가** ② **그 반복 사용이 비용·위험을 포함해도 여전히 이득인가**.

**STOP 조건** — 실사용자 0 · 반복 사용 0 · 비용 > 성공작업 가치 · 권한/보안 위험 통제 불가 · 현업 적용마다 중앙팀이 붙어야 함 · **숫자는 늘었는데 업무 결과 개선 증거가 없음**.

> ⛔ **과장 방지**: 기사의 사용률 97.6%는 **회사 설문 자기보고**이며 생산성·품질 개선을 직접 증명하지 않는다(`PARK` — 성과 지표로 인용 금지). AI First 선언·예산은 *실행 의지*의 증거이지 *채택 성공*의 증거가 아니다. 패널 발언은 전체 녹취가 아니므로 문맥 확장 금지.
> 출처: NDC26 AX 대담 보도(gamemeca, HTTP 200 실독) + NDC 공식 세션 페이지(200, 세션·연사 확인). Krafton 공식 릴리스는 curl 403 → 고수준 대조에만 사용. 사실 재료 자체는 [게임업계 사내 AI 조직·도구 지형 (2025~2026)](game-industry-ai-org-landscape.md)에 이미 흡수돼 있고, 본 절은 **재조합 원칙만** 병합한 것이다(새 사실 노트 미생성).
