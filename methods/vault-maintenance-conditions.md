...
> 🔁 **재발 2026-08-11 (recurrence 2)** — 같은 함정을 다시 밟았다. `code` 102개만 AST → 3,906노드 · 캐시 4,111 합쳐 8,017 = 정본 24,461 대비 **-66%**. 전량(code+document+paper 1,725파일) 재실행 → **24,766**. 🩸 **본 R-7 을 읽지 않고 처음부터 재도출했다** — graphify 작업 착수 시 스킬(외부)만 로드하고 vault 를 조회하지 않았다(CLAUDE.md ① Dispatch 철칙 *"vault 조회 없이 실질 작업 없음"* 위반, red-flag *"새 도메인이면 mid-session 재조회"* 해당). **즉 이 항목의 실패는 지식 부재가 아니라 lookup 누락이다.** 처방 추가: **graphify·재빌드 착수 전 본 노드 R-7~R-9 를 먼저 읽는다**(`grep -n "R-[789]" wiki/learnings/methods/vault-maintenance-conditions.md`). 가설 확인 = "캐시 열화 -73%"의 실체가 이 라우팅 문제라는 R-7 원문 가설은 2회 실측으로 **강화**(미확정 → 유력).
...
> 🔁 **부분 재발 2026-08-11 (recurrence 2)** — 4청크 중 **세션 한도로 2개 사망**, R-8 처방대로 *그대로 재실행*(1건은 `SendMessage` 로 트랜스크립트 이어붙이기, 1건은 에이전트 ID 부재로 재디스패치)해서 전건 회수. ⭐ **R-8 에 추가할 델타 1건**: chunk04 는 한도로 죽었는데도 **죽기 전에 JSON 을 먼저 써서 살았다**. 즉 실패 유형과 무관하게 *산출물 우선 기록*이 생존율을 올린다 → 외주 프롬프트 고정 슬롯: **"여유가 없으면 적은 파일이라도 제대로 추출하고, 끊기기 전에 JSON 을 먼저 써라. 그 파일이 유일한 성공 신호다."** 실측: 그 문구를 넣은 재디스패치는 전건 완료 + 미독 구간을 정직 보고(919줄 중 330줄만 읽음을 명시하고 미독분 노드 생성 0). ⚠️ 반면 출력 예산(`~40-70 노드`) 명시는 이번에 **누락**했다 — 이번엔 출력 초과가 없어 손해가 없었을 뿐이다.
...
> 🔁 **재발 2026-08-11 (recurrence 2) + 근본 원인 확정.** 07-31 에 상대경로로 재기록해 닫았는데 08-10 에 다시 3,453 엔트리(절대 1,797 + 레거시 상대 1,656)로 깨져 있었다. **왜 재발했나 = `save_manifest(files)` 를 `root` 없이 부르면 절대키로 쓴다.** 함수 docstring 실측: *"When `root` is provided, keys are relativized against it before write (forward-slash, posix-style) so the on-disk manifest is portable across machines"*. **graphify 스킬 Step 9 는 `root` 를 넘기지 않는다** → 매 실행이 절대키를 상대키 위에 얹는다 = 사후 재기록으로는 절대 안 닫히는 구조. ⛔ **처방 교체(사후 수정 → 호출 규약)**: 재빌드 Step 9 는 반드시 `save_manifest(files, root=Path('.').resolve())` 로 부른다. 그러면 키가 처음부터 상대라 다음 실행이 같은 키를 덮어써 증식하지 않는다.
> 🩸 **내 08-11 우회 1건(정정 기록)** — 원인을 모른 채 *절대키만 남기고 상대키를 지우는* 반대 방향으로 고쳤다. 체커는 통과했지만 ① 본 R-9 의 확립된 방향과 어긋나고 ② `root` 미전달이라는 진짜 원인을 가리고 ③ portable 성질(#777)을 잃는다. 원인 규명 후 상대키로 되돌렸다(절대키 잔존 0 확인). **교훈: 게이트가 초록이 됐다는 것이 원인을 찾았다는 뜻은 아니다.**

**R-10. 증분 갱신 정식 경로 — 상류 `--update` 병합의 대체(2026-09-13 첫 실사용)** <!-- proposed_by: claude · status: provisional (작가 지시 「증분이 기본, 구조를 고쳐라」 09-13 의 실행 기록 — 절차 내용은 작가 검토 전) -->
상류 병합(R-2)을 고치지 않고 우회하되 *매번 전체 재구축*으로 우회하지 않는다. 경로 = `scripts/graphify-incremental-plan.py plan`(링크 경로만 바뀐 파일은 옛 추출 이월 — git 옛 본문 md5 가 목록 파일 기록값과 같을 때만) → 새로 읽을 문서만 모델 추출 → `scripts/graphify-incremental-run.py post`(합집합 + 삭제·범위 밖 파일 노드만 source_file 기준 제거, 가드 `--scope-prune` 독립 재판정) → `build --guard-passed` → `finalize`(목록 root 지정 = R-9 · 이월 조각 캐시 승격). **실측**: 추출 대상 ~~1,215~~ → **89**(문서 75 = 서브에이전트 9묶음, 합계 약 176만 토큰 · 구조 추출 324파일 무료) · 노드 40,573 → **38,480**(범위 밖 3,590·삭제 15 제거 · 보호 자산 atlas +9 · creative +144 · me +9 무손실) · `graph_freshness` HEALTHY(changed·new·missing 0). ⚠️ 비용의 절반 이상은 큰 산출물 3개(PDF 6.6MB·HTML 6MB·2.6MB)와 9월 보관 기록(289KB)이 차지했다 — 다음 레버는 범위다. 설계·게이트 = RETURN · GATE.
