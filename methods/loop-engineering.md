...
- ⭐ **7.1 잔량-시간 페이싱 (quota window pacing)** — 절대 캡만으로는 부족하다. **쿼터가 창(window)으로 리셋되는 요금제**(Claude 5h/7d 등)에서는 *남은 양*을 *남은 시간*과 **비교**해야 한다.
  ```text
  left     = (100 - usedPercent) / 100      # 남은 쿼터 비율
  timeLeft = minutesToReset / windowMinutes # 남은 시간 비율
  left < timeLeft - m   →  시간 대비 잔량 부족 = 아껴 써야 하는 구간
  ```
  판정은 이진(있다/없다)이 아니라 **`큰 작업 / 작은 작업 / 대기 / 루프 중지` 4단 권고**로 낸다. `left ≈ timeLeft` 면 페이스가 맞는 것이므로 잔량이 적어도 조일 필요가 없다 — **절대 잔량만 보면 멀쩡한 구간에서 헛되이 조인다.**
  ⚠️ 5h 창과 7d 창은 **성격이 다르다**(단기 스파이크 ⟂ 장기 소진). 같은 공식을 쓰되 임계 `m` 을 따로 잡고, 창이 여러 개면 **가장 빡빡한 하나로 통합**해 판정한다.
  <!-- 출처: Codex `usage-coach` 해체 ③Gate 2026-08-05(repo netwaif/usage-coach `4c8368f0…`, MIT). 도구 미설치 — 정책 레이어 개념만 흡수. 원 저장소가 겨냥한 축은 "사용량 표시"가 아니라 `agent-loop budget policy`. -->
  ⛔ **미검증**: 임계 `m` 의 적정값·4단 권고가 실제로 소진을 줄이는지 실측 0(원 repo star 5·릴리스 없음). 공식은 산술적으로 자명하나 **효과는 주장이 아니다.**
...
## 설명용 4-루프 스택 (2026-07-25 ③Gate — LangChain/LangSmith 자료 델타)

위 14단계가 *실행* 아키텍처라면, 이건 **남에게 설명할 때 쓰는 압축 모델**이다. 자기 판정도 그렇다 — *"새 사실이 아니라 에이전트 운영을 설명하는 더 선명한 멘탈 모델"*.

- **4개 루프 각각에 계측 primitive 1개를 붙인다** — 루프마다 "무엇으로 관측하나"가 짝지어져야 한다.
- **trace → harness-config 개선 루프를 명시한다** — 트레이스가 로그로 끝나지 않고 *설정을 바꾸는 입력*이 되는 경로.
- **루프별 human oversight 지점을 따로 표기한다** — 어느 루프에서 사람이 개입하는지를 루프 단위로 못 박음.

> **기존 노트가 더 강한 부분은 그대로**: durable execution·안전·infra-0 triage 는 본 노드가 원자료보다 앞서 있다. 이 델타는 *교육·설명* 축의 보강이지 아키텍처 교체가 아니다. 신규 skill 미제안(`loop-engineering`·agent-harness·hermes-loop 가 이미 포괄).

<!-- 출처 URL 이 frontmatter 에 없던 신규 소스. link-rot 증거 동봉. 신규 노드 대신 델타 병합. -->
