...
- "queue not loops" + 사람 triage = 우리 hermes-loop·[Loop Engineering (Addy Osmani) — 하네스 위 한 층, 스케줄 발동 loop (vault 수렴 ground-truth #6)](loop-engineering.md) Automations 가드(read-only·작게·triage inbox)와 정합. infra-0 큐 철학 확증.
...
- **Sandcastle 도구** = dissect-not-install-external-tools 점검 대상(infra-0·TS/Node 의존). 지식 확증만, 도입 X.
