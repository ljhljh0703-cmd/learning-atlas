...
- **그래프는 read model 이고 권위는 소스에 있다** 🔄 2026-08-10 보강 <!-- 출처 = Ix(ix-infrastructure/Ix `52c18a3`) 해체 · 패킷 `~/Documents/Codex/2026-08-08/x-loopx-ix-prime-agent-teardown/` · draft: external_ai (via codex), gate: vault Claude. 대조: 위 "Stale 인덱스 = 증거 X" 는 *조회 전 상태 확인*을 규정하고, 본 항은 *응답이 자기 출처를 들고 다니게* 한다 — 별개 층 -->: 파생 코드그래프를 `shared source of truth` 로 말하지 않는다. 권위는 **소스 코드와 현재 런타임 증거**이고 그래프는 그 위의 projection 이다. 따라서 구조 질의의 결과를 인용할 때는 **`graph 리비전·source commit·확신도·source anchor(파일:라인)` 를 함께 들고 온다** — 앵커 없는 구조 답변은 "예쁜 다이어그램이 미문서화 권위로 승격"하는 경로다. 실무 형태: 그래프로 *좁히고* 소스로 *확인*한다(`구조 질의 → 소스 앵커 → 실독 검증`). 위 stale 규율이 *언제 믿지 말지*라면 이 항은 *무엇을 함께 제시할지*다.
  - ⛔ **Graft(NanoNets) 도입은 기각**(`SKIP` · 설치 0) 🔄 2026-08-15 <!-- 출처 = repo `d834ca22` shallow clone + X 원문 복원 해체 · 패킷 `~/Documents/Codex/2026-08-14/four-source-independent-teardown/` · draft: external_ai (via codex), gate: vault Claude ③Gate -->: tree-sitter 구조 그래프 + Claude Code hook 으로 관련 노드를 prompt 앞에 **push** 하는 도구. 기존 CodeGraph 레인과 역할 중복이고, 자동 주입은 authority risk(§"그래프는 read model" 과 정면). **가져온 것은 평가 설계 1종뿐 — `push context`(hook 선주입) ⟂ `pull tool`(도구 제공)을 별도 arm 으로 비교하고, efficiency QA ⟂ real implementation benchmark 를 분리해 tool-call·token·wall-clock·correctness 를 동시 측정.** ⚠️ **홍보 수치는 서로 다른 실험을 한 묶음처럼 섞었다** — `+12 points` 는 SWE-bench Verified **50개 subset**(27/50→33/50), `−42% tokens/−46% tool calls/−60% time` 은 **별도 162-run 2-repo QA**(그 실험의 correctness 는 93%=93% 무변화)이고 token 절감(−42%)과 달러 절감(−32%)도 다르다. pin 된 clone 에 benchmark script·task list·raw transcript·result artifact **0** → publisher-reported·independently unreproduced. `DeepSWE +20%` 는 영상 문구만 존재. 도입 재개 조건 = 미래 게임 repo 에서 repeated exploration 이 실측될 때 CodeGraph ⟂ Graft 중 **하나만** held-out pilot(동시 설치 금지 · pass@K 상승은 채택 근거 아님 · `graft init` 이 `.claude/` wiring 과 `.gitignore` 를 수정하므로 `--dry-run` + clean clone 감사 선행).
