...
- **썸네일** 1100×800 PNG, 핵심 화면, 텍스트 과다 X
  > ⚠️ **Contradiction — 미해결(2026-08-05).** Codex 패킷 `jelly-panic-rating-route-update`(캡처 2026-07-23)는 현행 공식 규격을 **1932×828** 로 보고한다. **어느 쪽도 확정하지 않았다** — ① 패킷이 인용한 URL(`/prepare/console-workspace.html`)은 2026-08-05 조회 시 **404** ② 내 독립 확인 시도는 **내 프롬프트가 후보 숫자를 제시해 답이 오염**됐다(모델이 그대로 되읊음 = 근거 아님). 원문 인용으로 확인된 건 로고 **600×600** 뿐(*"600×600px의 각진 정사각형"*).
  > ▶ **자산 제작 전 콘솔 등록 화면에서 실물 확인 필수.** 1100×800 로 만들면 반려 위험, 1932×828 로 만들어도 근거 없음. 해소 = 콘솔 스크린샷 1장.
