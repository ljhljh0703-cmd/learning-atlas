...
## 6. 재점검 delta (2026-07-18 — 작가 "새 업데이트/참고할 거 체크")

현재 repo(v0.41.37.0, 412 커밋, ⭐26.5k) vs 기존 흡수분 대조. 대부분 **인프라 성숙(infra-0, 흡수 불요)**. 진짜 신규 = 3개 (전부 **park**):

- **Schema packs** (페이지 타입 택소노미 커스터마이징 팩) — vault 카테고리(me/experiences/thoughts…)를 모듈 팩화하는 발상. [MemoHarness — 경험에서 학습하는 에이전트 하네스](memoharness-experience-learning.md) D1~D6 taxonomy와 공명. **park** — 트리거: vault 카테고리 재편 필요 체감 시(현재 불요·Karpathy #2).
- **MCP 멀티에이전트 접근** (Claude Code·Codex·Cursor·ChatGPT·Perplexity가 gbrain을 MCP로 조회) — ⭐ remote-subbrain-outpost-plan의 **참조 구현**. **park** — 전초기지 OQ-2(진단·솔루션 메커니즘)에 배선(그쪽 문서에 파킹).
- **brains ⊥ sources 2축** (brains=DB, sources=brain 내 repo들) — 단일 vault repo 대비 조직 대안(취업/게임/창작 = sources). **park** — 트리거: vault 단일 repo 확장성 한계 체감 시.

한계·비판(변함없음): 여전히 상용 SaaS 인프라 헤비(PGLite·Minions·OAuth·reranker·multi-user) = 무서버 vault엔 과잉. 자체측정 미검증. fail-open 가드 = vault fail-safe와 정반대. → **delta 좁음 재확인**: vault가 이미 수렴점. 역수출 후보 = schema packs·MCP 접근 개념 2개뿐.
