...
## PDF 정독 델타 (2026-07-25 ③Gate — arXiv 2604.12946 원문 44p)

초판은 요약 기반이었다. 원문 `pdftotext` 2,434줄 + 페이지 렌더 실측으로 보강한 부분.

- **LTI/시스템 관점** — 루프를 선형 시불변계로 보면 `h_{t+1} = A·h_t + B·e + R(h_t, e)`, 선형화 안정 조건은 **`ρ(A) < 1`**(스펙트럼 반경). "왜 안 터지나"의 답이 여기 있다.
- **prior 주입 방식 비교** — *덧셈* = 한계 안정(marginally stable) · *concat* = 무제약·불안정 · **Parcae = 안정**. 루프 설계에서 주입 방식이 안정성을 가른다.
- **3중 안정화** — ①제약된 `A` ②per-sequence stochastic depth ③prelude norm. 셋이 같이 있어야 성립.
- ⚠️ **test-time 포화** — 루프를 늘려도 무한정 좋아지지 않는다. **`L(T) = L_inf + Z·exp(-z·T)`** 로 수렴 — *루프 수 = 성능*이 아니다.
- ⭐ **AI NPC 오독 방지** — Parcae 식 "생각 루프"는 **아키텍처 수준의 순환**이지 **프롬프트 자기성찰이 아니다.** 둘을 같은 것으로 쓰면 범주 오류. → ai-npc-blueprint 적용 시 이 구분 유지.
- 📌 **출처 등급 교정** — 위 §기술 스택의 `pip install parcae-lm` 스니펫은 **GitHub/소스 파생**이지 PDF 파생이 아니다. 별도 재검증 전까지 논문 근거로 인용하지 말 것.

<!-- 소스 락: PDF SHA-256 b202a3bd286eac8acec3f5679033b62403159ac99ffc82816f715247ba1fd78c · arXiv abs·PDF 200 · 44p 비암호화. 신규 노드 생성 대신 델타 병합(중복 방지). -->
